...
// 实现基础函数
int tcpx_load_plugin(const char* plugin_path) {
...
int tcpx_get_device_count() {
    if (!tcpx_initialized) {
        ...
        if (tcpx_load_plugin("/usr/local/tcpx/lib64/libnccl-net-tcpx.so") != 0) {
            printf("[TCPX] Failed to load default plugin, returning 1 device\n");
            return 1;
        }
    }
    
    int ndev = 0;
    if (tcpx_devices_func && tcpx_devices_func(&ndev) == 0) {
        printf("[TCPX] Found %d TCPX devices\n", ndev);
        return ndev;
    }
    
    printf("[TCPX] Failed to get device count, returning 1\n");
    return 1;
}
