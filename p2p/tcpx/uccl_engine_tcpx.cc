...
struct uccl_engine {
    std::unique_ptr<tcpx::TcpxEndpoint> endpoint;
};
...
uccl_engine_t* uccl_engine_create(int local_gpu_idx, int num_cpus) {
    try {
        ...
        eng->endpoint = std::make_unique<tcpx::TcpxEndpoint>(local_gpu_idx, num_cpus);
        return eng;
    } catch (const std::exception& e) {
        fprintf(stderr, "[TCPX] Engine creation failed: %s\n", e.what());
        return nullptr;
    }
}
...
uccl_conn_t* uccl_engine_accept(uccl_engine_t* engine, void* ip_buf, size_t ip_buf_size,
                                int* remote_gpu_idx) {
...
// Memory registration
uccl_mr_t* uccl_engine_reg(uccl_engine_t* engine, uintptr_t data_ptr, size_t size) {
    if (!engine) return nullptr;
    
    auto* mr = new uccl_mr;
    mr->engine = engine;
    
    bool ok = engine->endpoint->reg(reinterpret_cast<void*>(data_ptr), size, mr->mr_id);
    if (!ok) {
        delete mr;
        return nullptr;
    }
...
int uccl_engine_write(uccl_conn_t* conn, uccl_mr_t* mr, void const* data, 
    ...
    bool ok = conn->engine->endpoint->write_async(conn->conn_id, mr->mr_id, 
                                                 data, size, dummy_addr, transfer_id);
    return ok ? 0 : -1;
}

int uccl_engine_recv(uccl_conn_t* conn, uccl_mr_t* mr, void* data, size_t size) {
...
int uccl_engine_advertise(uccl_conn_t* conn, uccl_mr_t* mr, void* addr, 
    ...
    bool ok = conn->engine->endpoint->advertise(conn->conn_id, mr->mr_id, addr, len, out_buf);
    return ok ? 0 : -1;
}

// Placeholder implementations for compatibility
int uccl_engine_get_fifo_item(uccl_conn_t*, void*) { 
    return -1; 
}
