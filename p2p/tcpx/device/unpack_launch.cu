...
int UnpackLauncher::launch(const tcpx::rx::UnpackDescriptorBlock& desc_block,
  ...
  if (use_staging) {
    if (dbg) {
      std::cout << "[Debug Kernel] [staging] DISABLED - staging buffer causes offset issues" << std::endl;
      std::cout << "[Debug Kernel] [staging] Reason: descriptors have src_off relative to original bounce_buffer" << std::endl;
      std::cout << "[Debug Kernel] [staging] Copying entire bounce buffer would require recalculating all offsets" << std::endl;
      std::cout << "[Debug Kernel] [staging] Proceeding without staging buffer..." << std::endl;
      std::cout.flush();
    }
    ...
    //    b) Copy from (bounce_buffer + min_offset) for (max_offset - min_offset) bytes
    //    c) Adjust all src_off values by subtracting min_offset
    // 4. This is complex and error-prone
    // 5. Better to let kernel read directly from bounce_buffer (which is already on GPU)
  }
