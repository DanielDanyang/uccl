...
int main() {
    ...
    if (device_count > 0) {
        ...
        std::cout << "\n[Step 2] Testing multiple calls to device discovery..." << std::endl;
        ...
        if (device_count2 == device_count) {
            std::cout << "✓ SUCCESS: Consistent device count on second call" << std::endl;
        } else {
            std::cout << "⚠ WARNING: Device count changed: " << device_count2 << std::endl;
        }
        ...
        std::cout << "⚠ WARNING: Plugin loaded but no TCPX devices found" << std::endl;
        std::cout << "This might be expected if no TCPX hardware is available" << std::endl;
        return 0;
        
    } else {
        std::cout << "✗ FAILED: Device discovery returned error: " << device_count << std::endl;
        std::cout << "\nTroubleshooting steps:" << std::endl;
        std::cout << "1. Check if TCPX plugin exists:" << std::endl;
        
        const char* plugin_path = getenv("UCCL_TCPX_PLUGIN_PATH");
        if (!plugin_path) {
            plugin_path = "/usr/local/tcpx/lib64/libnccl-net-tcpx.so";
        }
        std::cout << "   Plugin path: " << plugin_path << std::endl;
        
        std::cout << "2. Try setting UCCL_TCPX_PLUGIN_PATH to correct path" << std::endl;
        std::cout << "3. Ensure TCPX plugin is properly installed" << std::endl;
        std::cout << "4. Check plugin dependencies (CUDA, network drivers)" << std::endl;
