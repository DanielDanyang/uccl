...
// NOTE(TCPX): RDMA-specific initialization; replace with TCPX transport.
Endpoint::Endpoint(uint32_t const local_gpu_idx, uint32_t const num_cpus)
  ...
  */
  ...
  // TODO(TCPX): Initialize TCPX transport instead of RDMA
  ep_ = nullptr;  // TCPX doesn't use ep_ - we'll use direct TCPX calls

  // Initialize TCPX device discovery
  int ndev = tcpx_get_device_count();
  if (ndev <= 0) {
    std::cerr << "TCPX: No devices found or failed to initialize TCPX plugin"
              << std::endl;
    throw std::runtime_error("Failed to initialize TCPX");
  }
  std::cout << "TCPX: Found " << ndev << " devices" << std::endl;

  // For now, use device 0 for all GPUs (simplified mapping)
  int ngpus_detected = 0;
  GPU_RT_CHECK(gpuGetDeviceCount(&ngpus_detected));
  for (int i = 0; i < std::min(ngpus_detected, kMaxNumGPUs); i++) {
    gpu_to_dev[i] = 0;  // Use TCPX device 0 for all GPUs
  }
  for (int i = ngpus_detected; i < kMaxNumGPUs; i++) {
    gpu_to_dev[i] = 0;
  }
  numa_node_ = 0;  // TODO: Get NUMA node from TCPX device info

  std::cout << "TCPX: Initialized for GPU " << local_gpu_idx_
            << " using TCPX device " << gpu_to_dev[local_gpu_idx_] << std::endl;
  ...
  */
  ...
  // TODO(TCPX): Initialize TCPX-specific task management and threading
  send_unified_task_ring_ = nullptr;
  recv_unified_task_ring_ = nullptr;

  std::cout << "Endpoint initialized successfully" << std::endl;
}

// NOTE(TCPX): RDMA teardown logic; interacts with uccl::RDMAEndpoint internals.
Endpoint::~Endpoint() {
  [[maybe_unused]] auto _ =
      PyGILState_Check() ? (py::gil_scoped_release{}, nullptr) : nullptr;
...
// NOTE(TCPX): RDMA bulk registration path; replace for TCPX.
...
// NOTE(TCPX): RDMA memory deregistration.
...
// NOTE(TCPX): RDMA send path using uccl::RDMAEndpoint.
...
// NOTE(TCPX): RDMA recv path using uccl::RDMAEndpoint.
...
// NOTE(TCPX): RDMA async send implementation.
...
// NOTE(TCPX): RDMA async recv implementation.
