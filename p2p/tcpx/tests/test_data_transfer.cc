...
int main() {
  ...
  std::cout << "This test verifies memory registration and data transfer APIs"
            << std::endl;
  ...
  std::cout << "Allocated " << buffer_size << " bytes at " << test_buffer
            << std::endl;
  ...
  if (rc_reg == 0) {
    std::cout << "✓ SUCCESS: Memory registered, mhandle=" << mhandle
              << std::endl;
    ...
    if (rc_dereg == 0) {
      ...
      std::cout << "✗ FAILED: Memory deregistration failed with rc=" << rc_dereg
                << std::endl;
    }
  } else {
    std::cout << "✗ EXPECTED: Memory registration failed with rc=" << rc_reg
              << std::endl;
    std::cout
        << "This is expected since we don't have a valid communication handle"
        << std::endl;
  }
  ...
  // Test send with null comm (should fail gracefully)
  std::cout << "Testing tcpx_isend with null comm..." << std::endl;
  int rc_send =
      tcpx_isend(dummy_comm, test_buffer, 100, 42, mhandle, &send_request);
  if (rc_send == 0) {
    std::cout << "✓ UNEXPECTED: tcpx_isend succeeded, request=" << send_request
              << std::endl;
  } else {
    std::cout << "✓ EXPECTED: tcpx_isend failed gracefully with rc=" << rc_send
              << std::endl;
  }

  // Test receive with null comm (should fail gracefully)
  std::cout << "Testing tcpx_irecv with null comm..." << std::endl;
  void* recv_data[1] = {test_buffer};
  int recv_sizes[1] = {100};
  int recv_tags[1] = {42};
  void* recv_mhandles[1] = {mhandle};

  int rc_recv = tcpx_irecv(dummy_comm, 1, recv_data, recv_sizes, recv_tags,
                           recv_mhandles, &recv_request);
  if (rc_recv == 0) {
    std::cout << "✓ UNEXPECTED: tcpx_irecv succeeded, request=" << recv_request
              << std::endl;
  } else {
    std::cout << "✓ EXPECTED: tcpx_irecv failed gracefully with rc=" << rc_recv
              << std::endl;
  }

  // Test completion check with null request (should fail gracefully)
  std::cout << "Testing tcpx_test with null request..." << std::endl;
  int done = 0, size = 0;
  int rc_test = tcpx_test(nullptr, &done, &size);
  if (rc_test == 0) {
    std::cout << "✓ UNEXPECTED: tcpx_test succeeded, done=" << done
              << " size=" << size << std::endl;
  } else {
    std::cout << "✓ EXPECTED: tcpx_test failed gracefully with rc=" << rc_test
              << std::endl;
  }
  ...
  std::cout << "- Memory registration/data transfer APIs are available"
            << std::endl;
  std::cout << "- APIs correctly reject invalid parameters" << std::endl;
  std::cout
      << "\nNext step: Test with actual connection handles from test_connection"
      << std::endl;
