...
// Get number of channels from environment (default: 1 for backward compatibility)
int get_num_channels() {
  const char* env = std::getenv("UCCL_TCPX_NUM_CHANNELS");
  if (!env) return 1;
...
int main(int argc, char** argv) {
  ...
  std::cout << "[DEBUG] === Multi-Channel TCPX GPU-to-GPU transfer test ===" << std::endl;
  
  if (argc < 2) {
    std::cout << "[DEBUG] Usage: " << argv[0] << " <server|client> [remote_ip]" << std::endl;
    std::cout << "[DEBUG] Environment variables:" << std::endl;
    std::cout << "[DEBUG]   UCCL_TCPX_NUM_CHANNELS: Number of channels (default: 1)" << std::endl;
    std::cout << "[DEBUG]   UCCL_TCPX_UNPACK_IMPL: d2d|host|kernel (default: d2d)" << std::endl;
    std::cout << "[DEBUG]   UCCL_TCPX_HOST_RECV_DEBUG: 1 to use host buffer" << std::endl;
  ...
  if (num_channels > device_count) {
    std::cout << "[DEBUG] WARNING: Requested " << num_channels 
              ...
              << " TCPX devices available. Clamping to " << device_count << std::endl;
  ...
  if (is_server) {
    ...
    if (mgr.get_num_channels() == 0) {
      std::cout << "[DEBUG] ERROR: ChannelManager initialization failed" << std::endl;
      return 1;
    }
    
    std::cout << "[DEBUG] ChannelManager created with " << mgr.get_num_channels() 
              << " channels" << std::endl;
    ...
    std::cout << "[DEBUG] Listening on " << handles.size() << " channels" << std::endl;
    ...
    std::cout << "[DEBUG] Sent " << handles.size() << " handles to client" << std::endl;
    ...
    if (mgr.register_memory(recv_buf, kRegisteredBytes, recv_ptr_type, true) != 0) {
      return fail_server("[DEBUG] ERROR: register_memory failed for server receive buffer");
    }
    ...
    if (!done) {
      ...
      auto* dev_handle_struct = reinterpret_cast<NcclNetDeviceHandle*>(ch0.recv_dev_handle);
    ...
    int bootstrap_fd = -1;
    if (bootstrap_client_connect(remote_ip, kBootstrapPort, &bootstrap_fd) != 0) {
    ...
    std::cout << "[DEBUG] Received " << handles.size() << " handles from server" << std::endl;
    ...
    std::cout << "[DEBUG] All " << mgr.get_num_channels() << " channels connected" << std::endl;
    ...
    if (mgr.register_memory(reinterpret_cast<void*>(d_aligned),
      ...
      return fail_client("[DEBUG] ERROR: register_memory failed for client send buffer");
    }
