 ...
 * 基于 test_tcpx_transfer.cc 的逻辑，增加了迭代和计时功能。
 *
 * 【核心设计】
...
constexpr size_t kHandleBytes = 128;        // NCCL handle 大小（用于 bootstrap 交换连接信息）
struct ncclNetHandle_v7 { char data[kHandleBytes]; };  // NCCL v7 handle 结构

constexpr int kBootstrapPort = 12347;       // Bootstrap TCP 端口（用于交换 TCPX handle）
constexpr int kTransferTag = 99;            // 基础传输标签（每个 chunk 会加上偏移）
constexpr size_t kMaxSize = 256 * 1024 * 1024;  // 最大传输大小（256MB）
constexpr size_t kRegisteredBytes = kMaxSize + 4096;  // 注册内存大小（额外 4KB 用于对齐）
...
int main(int argc, char** argv) {
  ...
  if (is_server) {
    ...
    // 2. 每个 chunk 的生命周期：irecv → kernel launch → event record → event sync → irecv_consumed
    // 3. 当窗口满时，等待最老的 chunk 完成，释放其请求槽，再发起新的 irecv
    //
    // 【为什么需要 CUDA events】
    // - 不能在 irecv 完成后立即调用 irecv_consumed（数据还在 bounce buffer，kernel 还没拷贝完）
    // - 必须等待 kernel 完成后才能调用 irecv_consumed
    // - 使用 CUDA event 跟踪每个 chunk 的 kernel 完成状态

    constexpr int MAX_INFLIGHT = 16;  // 最大并发 chunk 数（等于 TCPX 请求池大小）
    ...
    if (!use_host_recv && impl == "kernel") {
      ...
      cfg.use_small_kernel = true;   // 使用优化的小 kernel（warp-per-descriptor）
  ...
  // ============================================================================
    ...
    //
    // 【与 Server 端的区别】
    // - Server: 需要 CUDA events 跟踪 kernel 完成，因为 irecv_consumed 必须在 kernel 完成后调用
    // - Client: 不需要 events，因为 send 请求在 tcpx_test 返回 done=1 时自动释放

    constexpr int MAX_INFLIGHT_SEND = 12;  // 最大并发 send 请求数（< 16，留余量）
