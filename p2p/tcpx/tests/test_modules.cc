...
void test_bootstrap_server() {
  ...
  // Create server
  int client_fd = -1;
  std::cout << "Waiting for client connection on port " << kBootstrapPort << "..." << std::endl;
...
void test_bootstrap_client(const char* server_ip) {
...
void test_channel_manager() {
  ...
  std::cout << "[PASS] Created manager with " << num_channels << " channels" << std::endl;
...
int main(int argc, char** argv) {
  ...
  if (argc == 1) {
    ...
    std::cout << "  Client: ./tests/test_modules client <server_ip>" << std::endl;
    ...
    std::cerr << "  Client test:  " << argv[0] << " client <server_ip>" << std::endl;
    return 1;
  }
