 ...
 */
static int send_exact(int fd, const void* data, size_t size) {
  size_t total_sent = 0;
...
int bootstrap_server_create(int port, int* client_fd) {
  ...
  if (listen_fd < 0) {
    std::cerr << "[Bootstrap] socket() failed: " << strerror(errno) << std::endl;
    return -1;
  }
  ...
  if (listen(listen_fd, 1) < 0) {
    std::cerr << "[Bootstrap] listen() failed: " << strerror(errno) << std::endl;
    close(listen_fd);
    return -1;
  }
  ...
  if (conn_fd < 0) {
    std::cerr << "[Bootstrap] accept() failed: " << strerror(errno) << std::endl;
    close(listen_fd);
    return -1;
  }
...
int bootstrap_server_send_handles(int client_fd, 
  ...
  std::cout << "[Bootstrap] Sending " << channel_count << " handles to client" << std::endl;
  ...
  std::cout << "[Bootstrap] Successfully sent " << channel_count << " handles" << std::endl;
  return 0;
}
...
int bootstrap_client_connect(const char* server_ip, int port, int* server_fd) {
  ...
  std::cout << "[Bootstrap] Connecting to " << server_ip << ":" << port << std::endl;
  
  if (connect(sock_fd, reinterpret_cast<sockaddr*>(&addr), sizeof(addr)) < 0) {
    std::cerr << "[Bootstrap] connect() failed: " << strerror(errno) << std::endl;
    close(sock_fd);
    return -1;
  }
...
int bootstrap_client_recv_handles(int server_fd, 
  ...
  std::cout << "[Bootstrap] Receiving " << channel_count << " handles from server" << std::endl;
  
  if (channel_count == 0 || channel_count > 64) {
    std::cerr << "[Bootstrap] Invalid channel count: " << channel_count << std::endl;
    return -1;
  }
  ...
  std::cout << "[Bootstrap] Successfully received " << channel_count << " handles" << std::endl;
  return 0;
}
